import { X, Plus, Pencil, Trash2, PanelLeft, Loader2 } from "lucide-react"

export default function Sidebar({ t, open, onToggle, onClose, history, setHistory, activeHistoryId, setActiveHistoryId, editingId, setEditingId, editingName, setEditingName, onNew, onLoad, isDark, runningId }) {
  ...
  return (
    <div style={{ position: "fixed", left: 0, top: 0, bottom: 0, zIndex: 40, display: "flex" }}>
      ...
      <div style={{
        ...
        {/* Indicateur de run en cours sur le rail */}
        {runningId && (
              ...
              history.map((entry, i) => {
                ...
                return (
                  <div key={entry.id} onClick={() => { onLoad(entry); setActiveHistoryId(entry.id) }}
                    style={{ borderRadius: 8, padding: "8px 10px", cursor: "pointer", background: isActive ? "color-mix(in oklab, var(--primary) 15%, transparent)" : "transparent", border: isActive ? "1px solid color-mix(in oklab, var(--primary) 40%, transparent)" : "1px solid transparent", transition: "all 0.15s" }}
                    onMouseEnter={e => { if (!isActive) e.currentTarget.style.background = "color-mix(in oklab, var(--fg) 5%, transparent)" }}
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6 }}>
                        <div style={{ minWidth: 0, flex: 1 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            ...
                            {isRunning && <Loader2 size={11} style={{ color: "var(--primary)", flexShrink: 0, animation: "spin 1s linear infinite" }} />}
                          </div>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}>
                            ...
                            {isRunning && (
                              <span style={{ fontSize: "0.62rem", color: "var(--primary)", opacity: 0.8 }}>...</span>
                            )}
                            {!isRunning && entry.avgScore && (
